...
export async function registerAuthRoutes(app: FastifyInstance) {
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      if (!validLangs.includes(language)) {
        return reply.status(400).send({ code: "INVALID_LANGUAGE", message: "Unsupported language code" });
      }
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      if (!dbUser) return reply.status(404).send({ code: "NOT_FOUND", message: "User not found", traceId: request.id });

      const valid = await bcrypt.compare(currentPassword, dbUser.passwordHash);
      if (!valid) {
        return reply.status(400).send({ code: "WRONG_PASSWORD", message: "Current password is incorrect", traceId: request.id });
      }
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const result = await sendPasswordResetEmail({ to: normalizedEmail, resetUrl, appUrl });

      // In dev mode (no SMTP), return the link so the UI can show it
      return reply.status(200).send({ ok: true, ...(result.devLink ? { devLink: result.devLink } : {}) });
    },
  );
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!resetToken || resetToken.usedAt || resetToken.expiresAt < new Date()) {
        return reply.status(400).send({ code: "INVALID_TOKEN", message: "Reset link is invalid or has expired", traceId: request.id });
      }
